...
export const hookFactory: AccountHookFactory = ({provider, ethereum, isLoading}) => () => {
    ...
    const handleAccountsChanged = (...args: unknown[]) => {
        ...
        if(accounts.length == 0){
            console.error("CONNECT TO WEB3 WALLET");
    ...
    return {...swr, connect, mutate, data, isLoading: isLoading || isValidating, isInstalled: ethereum?.isMetaMask || false, isValidating};
};
